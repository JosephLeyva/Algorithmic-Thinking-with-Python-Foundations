...
unvisited_min_distances = {vertex: INFINITY for vertex in graph_1}
visited_vertices = {}
...
while len(unvisited_min_distances) > 0:
    ...
    current_vertex, current_distance = sorted(
        unvisited_min_distances.items(), key=lambda x: x[1])[0]  # Very inefficient - use priority queue in "real" version

    # For each unvisited neighbour of the current vertex
    for neighbour, neighbour_distance in graph_1[current_vertex].items():
        # If a neighbour has been processedm skip it
